...
main:   MOV R2, #(UART0_BIT)
	...
	MOV R1, #0xFF ; máscara das funções especiais no port A (bits 1 e 0)
        MOV R2, #0x11  ; funções especiais RX e TX GPIO_enable no port A (UART)
        BL GPIO_select
...
UART_ff: 
       CMP R1, #0x0D ; 
       BNE reply_end 
       LDR R2, =RESP ;constante de string
       MOVS R3, #27   ;constante do tamanho da string
      
;o loop fecha o envio, verifica se o transmissor está vazio.
loop_encerramento      
       LDR R4, [R0, #UART_FR] 
       TST R4, #TXFE_BIT 
       BEQ loop_encerramento
       LDR R5, [R2]
       STR R5, [R0]
       ADD R2, R2, #1
       SUB R3, R3, #1
...
RESP                   DC8     "Sistemas Microcontrolados\r\n" 
